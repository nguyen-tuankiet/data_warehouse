...
class ConfigManager:
        ...
        return {
            "booking": {
                ...
                "field_mappings": {
                    ...
                    "flight_number": {
                        "selectors": [
                            './/div[@data-testid="flight-number"]',
                            './/span[contains(@class, "flight-number")]',
                        ],
                        "required": False,
                        "type": "text",
                    },
                    "departure_time": {
                        "selectors": [
                            './/div[@data-testid="departure-time"]',
    ...
    def get_active_configs(self) -> List[Dict[str, Any]]:
        ...
        for provider, config in self.configs.items():
            if config.get("is_active", True):
                active_configs.append({"provider": provider, "config": config})
    ...
    def update_field_mapping(
        self, provider: str, field_name: str, mapping_data: Dict[str, Any]
    ) -> bool:
