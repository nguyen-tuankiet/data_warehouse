...
class BookingApiScraper:
    ...
    def scrape_flights(self, origin, destination, search_date):
        """Main scraping function for Booking API"""
        print(f"Starting Booking API scraping: {origin} -> {destination}")
        ...
        try:
            ...
            print(f"Booking API scraping completed. Found {len(flights)} flights.")
            return flights
            
        except Exception as e:
            print(f"Error during Booking API scraping: {e}")
            return []
